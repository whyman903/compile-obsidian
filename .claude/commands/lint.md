...
4. Do an editorial pass on article pages. For each article, check:
...
5. Run `compile obsidian refresh` after all fixes.

6. Run `compile health` again to confirm issues are resolved.

7. Report: issues found, issues fixed, remaining issues that need user input.
