...
final class ClaudeQueryRunnerTests: XCTestCase {
    ...
    func testSystemPromptDirectsReadOnlyAgentSearch() {
        ...
        XCTAssertTrue(prompt.contains("Every factual claim must be grounded"), "should require grounding every claim")
        XCTAssertTrue(prompt.contains("a response with no wikilinks is a failure"), "should make no-citation responses a failure")
        XCTAssertTrue(prompt.contains("Do not answer from prior knowledge"), "should forbid ungrounded prior-knowledge answers")
