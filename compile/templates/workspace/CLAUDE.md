...
2. **Create the source note.** Run `compile ingest <filename>` to create a source note with provenance, key sections, and likely related pages. Then read the raw source and continue the maintenance flow: strengthen the source note where it still needs deeper synthesis or caveats, and update related articles in the same pass.
